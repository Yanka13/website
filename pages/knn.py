...
if uploaded_file is not None:
    ...
    # find_k_neighbours
    result = find_k_neighbours(image = img, file_location = "raw_data/wikiart_scraped.csv")

    col1, col2, col3, col4 = st.columns(4)

    with col1:
        image1 = skimage.io.imread(f"raw_data/abstract_ex/{result[0]['Image_filename']}", as_gray=False)
        st.image(image1)
        st.write(f"{result[0]['Title']}" )
        st.write(f"by {result[0]['Artist']}")

    with col2:
        st.image(f"raw_data/abstract_ex/{result[1]['Image_filename']}")
        st.write(f"{result[1]['Title']}")
        st.write(f"by {result[1]['Artist']}")

    with col3:
        st.image(f"raw_data/abstract_ex/{result[2]['Image_filename']}")
